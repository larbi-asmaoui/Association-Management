<template>
    ...
    <div class="bg-white pt-6 shadow-md rounded-xl relative mt-5">
        ...
        <div class="mt-7 px-2 items-center justify-between block sm:flex md:divide-x md:divide-gray-100">
        ...
        <teleport to="body">
            <Modal size="xl" v-if="isModalOpen" @close="closeModal">
                ...
                <template #body>
                    <form class="space-y-2 px-2 lg:px-2 pb-2 sm:pb-2 xl:pb-2 overflow-y-auto max-h-[30rem]"
                        @submit.prevent="submit">
                        <div>
                            <label for="title" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">Titre
                        ...
                        <div>
                            <label for="start" class="block text-sm text-gray-700 font-medium dark:text-white">Date de début
                            </label>

                            <input :min="new Date().toISOString().split('T')[0]" v-model="form.start" lang="fr-FR"
                                type="date"
                                ...
                                placeholder="Select date" name="start" />
                            <span v-if="form.errors.start" class="text-xs text-red-600 mt-1"
                                id="hs-validation-name-error-helper">
                                {{ form.errors.start }}
                            </span>
                        </div>
                        <!--  -->
                        <div>
                            <label for="end" class="block text-sm text-gray-700 font-medium dark:text-white">Date de Fin
                            </label>

                            <input lang="fr-CA" :min="new Date().toISOString().split('T')[0]" v-model="form.end" type="date"
                                class="py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700"
                                placeholder="Select date" name="end" />
                            <span v-if="form.errors.end" class="text-xs text-red-600 mt-1"
                                id="hs-validation-name-error-helper">
                                {{ form.errors.end }}
                            </span>
                        </div>
                        <div>
                            ...
                            <textarea v-model="form.description" rows="3" name="description" id="description"
                        ...
                        <div>
                            <label for="type" class="text-sm font-medium text-gray-900 block mb-2 :text-gray-300">Type
                            </label>
                            <select v-model="form.evenement_type_id" id="evenment_type_id"
                                ...
                                <option v-for="evenementType in evenementTypes" :key="evenementType.id"
                                    :value="evenementType.id"
                                    class="bg-white :bg-gray-800 py-2.5 px-4 cursor-pointer hover:bg-gray-200 :hover:bg-gray-700">
                                    {{ evenementType.name }}
                                </option>
                            </select>
                            <span v-if="form.errors.evenement_type_id" class="text-xs text-red-600 mt-1"
                                id="hs-validation-name-error-helper">
                        ...
                        <div>
                            <label for="adherents"
                                class="text-sm font-medium text-gray-900 block mb-2 :text-gray-300">Participants</label>
                            <Multiselect v-model="form.adherents" mode="tags" :close-on-select="false" :searchable="true"
                                :create-option="true" :options="formattedAdherents" />
                        </div>
                        <!-- **********  Select  ********** -->

                        <div>
                            <label for="location"
                                class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">Lieu de
                                l'événement</label>
                            <input v-model="form.location" type="text" name="location" id="location"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                required />
                        ...
                        <div>
                            <label for="type"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Région</label>
                            <select v-model="form.region" id="type"
                                ...
                                <option v-for="region in regions" @change="filterCities" :key="region.id"
                                    :value="region.name">
                                    {{ region.name }}
                                </option>
                            </select>
                            <span v-if="form.errors.region" class="text-xs text-red-600 mt-1"
                                id="hs-validation-name-error-helper">
                                {{ form.errors.region }}
                            </span>
                        </div>
                        <div>
                            ...
                            <select v-model="form.city" id="type"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                <option disabled value="">
                                    Séléctionner ville
                                </option>
                            ...
                            <span v-if="form.errors.city" class="text-xs text-red-600 mt-1"
                                id="hs-validation-name-error-helper">
                                {{ form.errors.city }}
                            </span>
                        </div>
...
const rows = computed(() =>
    Object.values(props.evenements).map((evenement) => ({
        id: evenement.id,
        reference: evenement.reference,
        start: evenement.start,
        end: evenement.end,
        adherents: evenement.adherents,
    }))
);
