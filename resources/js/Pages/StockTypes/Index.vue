...
<template>
    ...
    <div class="mt-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <the-card v-for="stockType in stockTypes" :key="stockType">
                <h5
                ...
                <p class="font-normal text-gray-700 dark:text-gray-400">
                    {{ $t("date_ajout") }} {{ stockType.created_at }}
                </p>
                <div class="flex justify-center mt-5 items-center">
                    <button
                        @click="openEditModal(stockType)"
                    ...
                    <button
                        v-show="stockType.stocks.length == 0"
                        @click="destroy(stockType)"
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route("stock-types.destroy", id), {
                onError: () => {
...
const openEditModal = (stockType) => {
    form.id = stockType.id;
    form.name = stockType.name;
    isModalOpen.value = true;
};

const submit = () => {
    if (form.id) {
        form.put(route("stock-types.update", form.id), {
            // forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
