        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <the-card v-for="revenueType in revenueTypes">
                ...
                <p class="font-normal text-gray-700 dark:text-gray-400">
                    Date d'ajout {{ revenueType.created_at }}
                </p>
...
const destroy = (id) => {
    if (confirm("vous êtes sûr?")) {
        form.delete(route("revenue-types.destroy", id), {
            onSuccess: () => {
                $toast.open({
                    message: "Type de revenue supprimé avec succès",
                    type: "success",
                    duration: 3000,
                    dismissible: true,
                });
...
const openEditModal = (revenueType) => {
    form.id = revenueType.id;
    form.name = revenueType.name;
    isModalOpen.value = true;
};

const submit = () => {
    if (form.id) {
        form.put(route("revenue-types.update", form.id), {
            // forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
                $toast.open({
                    message: "Type de revenue mis à jour avec succès",
                    type: "success",
                    duration: 3000,
                    dismissible: true,
                });
        ...
        form.post(route("revenue-types.store"), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
                $toast.open({
                    message: "Type de revenue ajouté avec succès",
                    type: "success",
                    duration: 3000,
                    dismissible: true,
                });
